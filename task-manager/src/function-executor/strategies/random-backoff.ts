  ...
  async execute<T>(func: ExecutiableFunction<T>): Promise<T> {
    ...
    while (retries < this.maxRetries) {
      try {
        ...
        this.logger.debug(`Function call failed. Calculated jitter ${jitter}, backoff ${backoff}, delay ${delay}. Based on retries ${retries}/${this.maxRetries}, base delay ${this.baseDelay}, jitter ${this.jitter}`);
        this.logger.log(`Function call failed with error: ${e.message}. Retrying in ${delay}ms (${retries}/${this.maxRetries})`);
        retries++;
        lastError = e;
